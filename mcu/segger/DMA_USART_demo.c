...
int main(void) {
    ...
    DMA1_Channel1->CCR |= _VAL2FLD(DMA_CCR_PL,0b10); // Channel priority high
    
    // Set DMA source and destination addresses.
    // TODO: Source: Address of the character array buffer in memory.
    DMA1_Channel1->CPAR |= _VAL2FLD(DMA_CPAR_PA, (uint32_t) &CHAR_ARRAY);

    // TODO: Dest.: USART data register
    DMA1_Channel1->CMAR |= _VAL2FLD(DMA_CMAR_MA, (uint32_t) &(USART->TDR));

    // TODO: Set DMA data transfer length (# of samples).
    DMA1_Channel1->CCR |= _VAL2FLD(DMA_CCR_MINC, CHAR_ARRAY_SIZE);
